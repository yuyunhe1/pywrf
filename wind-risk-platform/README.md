...
- 水平风切变属于同一高度层相邻节点之间的边属性。使用边两端 `u/v` 风矢量差除以 Haversine
  实际水平距离，并换算为每 1 km 风矢量变化；超过 `hard_delta_v_1km_ms` 时只禁止该边，
  不会把边两端节点整体设为不可通行。
- 风向变化角可作为同一水平边的辅助阈值；即使触发也只禁止该边，不会封锁端点节点。
- 垂直风切变当前停用，不参与节点可飞判断、路径搜索或高风险比例，也不会为此加载相邻高度层。
- WRF 水平切变代表约 3 km 模式网格尺度，不代表建筑或百米尺度的局地切变。

实验性默认阈值位于 `backend/config/wind_shear.json`。原有高风险比例由风速节点采样与实际经过的
